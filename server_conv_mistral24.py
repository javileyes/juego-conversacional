        ...
        for chunk in response:
            ...
            if idioma == "es":
                ...
                if trozo in ",;:.?!" and len(parte_actual)> minimo_caracteres:# or trozo in "." and len(parte_actual)>1:
                    # printf(HISTORICO_LOG, f"TROZO generado para USER: {userID}:", parte_actual)
                    if indiceParte == 0: #creamos primer audio rápido para rápida respuesta
                        estado_generacion[userID].primer_audio = voz_sintetica_spanish(parte_actual)
...
if idioma == "en":
    ...
    def voz_sintetica_spanish(text):
        ...
        printf(HISTORICO_LOG, f"texto preprocesado voz español:{text}\n")
